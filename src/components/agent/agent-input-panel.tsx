...
import { ExamplePrompt, type AgentType, FormattedStep } from "@/lib/types";
...
export function InputPanel({
  ...
  return (
    <motion.div
      ...
      animate="animate"
      exit="exit"
      className="w-full md:w-1/2 mx-2 lg:m-2 rounded-b-2xl rounded-t-sm 
                 lg:rounded-2xl p-[1px] bg-white flex flex-col 
                 h-[calc(100vh-6rem)] shadow-[0px_1px_1px_0px_rgba(0,_0,_0,_0.05),
                                           _0px_1px_1px_0px_rgba(255,_252,_240,_0.5)_inset,
                                           _0px_0px_0px_1px_hsla(0,_0%,_100%,_0.1)_inset,
                                           _0px_0px_1px_0px_rgba(28,_27,_26,_0.5)]"
    >
      {/* Examples */}
      <ExampleButtons
      ...
      <div className="flex flex-col flex-1 min-h-0">
        ...
        >
          {steps?.filter((step) => step.tool === "prompt" || step === steps[steps.length - 1]).map((step, idx) => (
            <div
              key={idx}
              ref={step.tool === "prompt" ? promptRef : null} 
              className={cn(
                "px-3 py-1.5 rounded-2xl shadow-sm break-words w-fit max-w-[75%]",
                step.tool === "prompt"
                  ? "ml-auto bg-neutral-300 text-neutral-600 text-sm"
                  : "mr-auto bg-neutral-100 text-neutral-800 text-sm"
              )}
            >
            ...
            {selectedAgent.inputFields.map((field, index) => (
              ...
              >
                <div className="flex items-center gap-1.5">
                  <Label className="text-xs font-medium text-neutral-600 transition-all duration-200">
                    {field.label}
                  </Label>
                  <AnimatePresence>
                ...
                {field.type === "textarea" ? (
                  <div className="relative p-2 border border-neutral-200 rounded-md shadow-sm bg-white">
                    <div className="ml-auto inline-block max-w-[75%] px-3 py-1.5 rounded-2xl 
                                    bg-neutral-200 text-neutral-600 text-sm shadow-sm break-words"
                    >
                      {inputs[field.name] || field.placeholder}
                    </div>
                  </div>
                ) : (
                  <Input
                    ...
                    className="text-[13px] border-none ring-1 ring-[#F6F6F6] ring-offset-[#F6F6F6] 
                              ring-offset-4 focus:border-neutral-300 focus:ring-neutral-200 
                              transition-colors duration-200 shadow-[0px_1px_0px_0px_hsla(0,_0%,_0%,_0.02)_inset,
                                                                _0px_0px_0px_1px_hsla(0,_0%,_0%,_0.02)_inset,
                                                                _0px_0px_0px_1px_rgba(255,_255,_255,_0.25)] 
  ...
  return (
    ...
    >
      <div className="">
        ...
        >
          <Label className="text-[11px] sm:text-xs font-medium text-neutral-500">
            QUICK EXAMPLES
          </Label>
        ...
        >
          ...
          <div className="mt-3">
            ...
            <div className="mt-1 bg-white border border-neutral-200 rounded-md px-2 py-1 text-xs text-neutral-700 shadow-sm">
              { selectedAgent.systemPrompt || "N/A" }
            </div>
          </div>
        </motion.div>
      </div>
      ...
      <div className="relative w-full">
        ...
        >
          {loading ? (
            ...
            <>
              <ArtificialIntelligence04Icon className="group-disabled:opacity-50 size-5 transition-all duration-200 ease-in-out group-disabled:fill-white text-red-400 stroke-1 fill-black mr-1.5" />
