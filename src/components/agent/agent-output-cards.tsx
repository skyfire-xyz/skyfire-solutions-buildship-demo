...
export function OutputPanel({
  ...
  return (
    ...
    >
      ...
      <div className="flex-none flex items-center justify-between w-full px-2 py-2 border-b border-border/50">
        {!loading && parsedOutput && (
          <CompletionIndicator
            status={parsedOutput?.error ? "error" : "success"}
            message={
              parsedOutput?.error
                ? `${parsedOutput?.message?.length > 50 ? parsedOutput?.message?.substring(0, 50) + '...' : parsedOutput?.message}`
                : "Tokens"
            }
...
// Keep the original AgentOutputCards for backward compatibility
